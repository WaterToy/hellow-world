# -*- coding: utf-8 -*-
"""
Created on Tue Feb 27 20:02:14 2018
...
df_no_holiday = df.loc[df.is_holiday==0][['date_time', 'is_holiday', 'gas_use', 'ave_temp']]
############################################### BP神经网络回归 #####################################################
# 训练数据归一化
train_X = df_no_holiday.ave_temp 
train_y = df_no_holiday.gas_use
...
# 训练 & 预测
pred_y = BP_Train_Predict(train_X=train_scaled[[0]], train_y=train_scaled[[1]], input_dim=1, test_X=test_scaled[[0]])
pred_data = pd.concat((test_scaled[[0]], pd.DataFrame(pred_y)), axis =1)
# 将预测数据反归一化，得回归结果
...
# design network
model = Sequential()
model.add(LSTM(45, input_shape=(LSTM_train_X.shape[1], LSTM_train_X.shape[2])))
model.add(Dense(LSTM_pred_days))
model.compile(loss='mae', optimizer='adam')
# fit network
history = model.fit(LSTM_train_X, LSTM_train_y, epochs=100, batch_size=45, validation_data=(LSTM_test_X, LSTM_test_y), verbose=2, shuffle=False)
# plot history
